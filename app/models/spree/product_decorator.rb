Spree::Product.class_eval do
  ...
  PRODUCT_SOLR_FIELDS << {:taxon_ids => {:as => :taxon, :multivalued => true}}
  acts_as_solr  :fields =>  PRODUCT_SOLR_FIELDS  , :facets => PRODUCT_SOLR_FACETS
  #, :include => [  {:taxons => {:as => :taxon,  :multivalued => true}} ]
  ...
  def taxon_ids
    @taxon_ids = []
    self.taxons.each.collect do |taxon| @taxon_ids << taxon.id end
    @taxon_ids
  end
